...
#想获取常规的一些信息可以执行“make help”
#更多的信息可以参见根目录下的README文件
#当前文件中的注释主要是给开发者看的，不要想通过阅读这个文件去了解怎样去构建内核

# o Do not use make's built-in rules and variables
#   (this increases performance and avoids hard-to-debug behaviour);
# o Look for make include files relative to root of kernel src
#不要使用make的build-in规则和变量（主要是为了提高性能并且避免难以调试的行为）
#
...
#我们正在使用递归构建，所以我们为了得到正确的需要做一点儿思考
...
# is now up to date.
#最重要的是：子Makefile只能在自己的目录中修改文件。 如果在某些目录中，
#我们依赖于另一个目录中的文件（通常不会发生这种情况，
#但是当将内置的目标终止为vmlinux时，通常是不可避免的），
#我们将在该目录中调用一个子代码 其他目录，
#之后，我们确信在其他目录中的所有内容现在都是最新的。

# The only cases where we need to modify files which have global
# effects are thus separated out and done before the recursive
# descending is started. They are now explicitly listed as the
# prepare rule.
#因此，只有在需要修改具有全局效果的文件的情况下才能分离出来并在递归递减开始之前完成。 它们现在被明确列为准备规则。
...
#通常情况下，我们会在指令执行前将其打印出来，通过echo $($(quiet)$(cmd))
#我们现在可以通过修改$(quiet)来选择其他输出的形式
#
...
#如果$(quiet)为空，则整条指令将被打印出来
#如果$(quiet) = "quiet_"，简短版本的指令将被打印出来
#如果$(quiet) = "silent_"，将不会打印任何文本
#因为变量$（silent_cmd_cc_o_c）不存在。

# A simple variant is to prefix commands with $(Q) - that's useful
# for commands that shall be hidden in non-verbose mode.
# 一个简单的变体是用$（Q）命令前缀 - 这对于在非详细模式下隐藏的命令非常有用。
#
#	$(Q)ln $@ :<
#
# If KBUILD_VERBOSE equals 0 then the above command will be hidden.
# If KBUILD_VERBOSE equals 1 then the above command is displayed.
# 如果KBUILD_VERBOSE  = 0，上述指令不会显示出来
# 如果KBUILD_VERBOSE  = 1，上述指令将会显示出来
#
# To put more focus on warnings, be less verbose as default
# Use 'make V=1' to see the full commands
# 为了更容易看到警告，在默认情况下不要使用'make V=1'去查看完整的指令
...
# 如果在终端里键入'make V=1'，将会给KBUILD_VERBOSE 赋值为1；
# 如果没有键入v的命令，默认KBUILD_VERBOSE = 0；
